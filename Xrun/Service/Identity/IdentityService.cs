...
namespace Xrun.Services.Identity
{
    public class IdentityService : IIdentityService
    {
       // private readonly IMongoCollection<UserDataEntity> _usersCollection;
        ...
        //_usersCollection = mongoDatabase.GetCollection<UserDataEntity>("UsersWithSalt");
        //}
             ...
             //   var user = await _usersCollection.Find(u => u.Email == loginReq.UserEmail).FirstOrDefaultAsync();
                ...
                if (user != null)
                {
                    if (PasswordWithSaltHashHelper.ValidatePassword(loginReq.UserPassword, user.PasswordToHash, user.Salt))
                    {
                        ...
                        //var update = Builders<UserDataEntity>.Update.Set(u => u.CurrJTI, jtiString);
        ...
        //=========== 有成功 就更新db refresh token   ==============
        {
            try
            {
               ...
               // var user = await _usersCollection.Find(u => u.Id == userId).FirstOrDefaultAsync();
                ...
                //====　↓　驗證通過　↓　====
                string jtiString = Guid.NewGuid().ToString();
                var update = Builders<UserAccountEntity>.Update.Set(u => u.CurrJTI, jtiString);
